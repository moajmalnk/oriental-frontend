    ...
    >
      <Card className="shadow-elegant border-0 overflow-hidden bg-gradient-card">
        <CardContent className="p-0">
          {/* Student Info Header */}
          {student.is_withheld && !isAuthenticated ? (
            <div className="bg-destructive/5 dark:bg-destructive/10 p-6 sm:p-8 text-center border-b border-border/50">
          ...
          {student.is_withheld && !isAuthenticated ? (
            <div className="p-8 text-center space-y-4 flex flex-col items-center justify-center min-h-[300px]">
              <div className="inline-flex h-20 w-20 items-center justify-center rounded-full bg-destructive/10 mb-2 animate-pulse">
                <AlertCircle className="h-10 w-10 text-destructive" />
              </div>
              <div className="space-y-2 max-w-md mx-auto">
                <h3 className="text-2xl font-bold text-destructive font-display">
                  Result Withheld
                </h3>
                <p className="text-muted-foreground leading-relaxed">
                  Your result is currently withheld due to administrative
                  reasons.
                  <br />
                  Please contact the office for clearance.
                </p>
              </div>
            </div>
          ) : (
            <div className="p-3 sm:p-4 md:p-6 lg:p-8 space-y-4 sm:space-y-6 md:space-y-8">
              {student.is_withheld && (
                <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-4 flex items-center gap-4 text-destructive mb-6">
                  <div className="p-2 bg-destructive/10 rounded-full">
                    <AlertCircle className="h-6 w-6" />
                  </div>
                  <div>
                    <h4 className="font-bold">Result Withheld</h4>
                    <p className="text-sm opacity-90">
                      This result is marked as withheld and is hidden from the
                      public view. You are seeing this because you are an admin.
                    </p>
                  </div>
                </div>
              )}
                                          ...
                                          const total = comboFields.reduce(
                                            (sum, field) => {
                                              ...
                                              switch (fieldKey) {
                                                ...
                                                case "PROJECT":
                                                  val =
                                                    subject.Project_Max || 0;
                                                  break;
                                                case "VIVA_PL":
                                                  val =
                                                    subject.Viva_PL_Max || 0;
                                                  break;
                                              }
                                              return sum + val;
                                            },
